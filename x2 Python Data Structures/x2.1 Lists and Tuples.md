...
Both lists and tuples are ordered data types (can be iterated over), are sequential, can contain multiple [types](/1%20Python%20Basics/1.2%20Types.md) and can be accessed via an index reference. Both are compound data types - data made up of multiple values or data types.  

The main difference between the two is that lists can be modified, whereas tuples cannot. This makes lists more flexible, however tuples are more memory efficient, making them the better option wehre the data will not need to change.
