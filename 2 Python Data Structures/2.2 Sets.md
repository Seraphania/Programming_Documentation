...
___
#### Remove
The `remove()` method can be used to remove an element from a set, however it The `remove()` method will only remove a single element at a time, and a `KeyError` will be raised if the item isn’t in the set. 
 ```py
set_1 = {1, 2, "apple", 3.5}
set_1.remove("apple")
print(set_1)
